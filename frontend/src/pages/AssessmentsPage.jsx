...
export const AssessmentsPage = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight flex items-center gap-2.5">
            Adaptive Skill Checkpoints <TrendingUp className="w-7 h-7 text-emerald-400" />
          </h1>
        ...
        {/* Quiz Selector Pills */}
        <div className="flex space-x-1.5 p-1 bg-slate-900 border border-slate-800 rounded-xl overflow-x-auto no-scrollbar">
      ...
      {assessment && (
        <Card className="glass-panel border-slate-800 p-6 sm:p-8 shadow-2xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800 mb-6">
            ...
            {!result && (
              <div className="flex items-center gap-4 shrink-0">
                <div className="flex items-center gap-1.5 px-3.5 py-1.5 bg-slate-900 border border-slate-800 rounded-xl font-mono text-xs font-bold text-amber-400">
          ...
          {result ? (
            <div className="space-y-6 animate-fade-in">
              ...
              >
                ...
                <div className="p-3 bg-slate-950/80 rounded-xl text-xs font-mono text-sky-300 flex items-center gap-2 border border-slate-800">
                  ...
                  <span>Adaptive Action: {result.adaptiveActionTaken}</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {result.reviewedQuestions?.map((q, idx) => (
                  <div key={idx} className="p-4 bg-slate-900/90 border border-slate-800 rounded-xl space-y-2">
                    <p className="text-xs font-semibold text-white">{idx + 1}. {q.questionText}</p>
                    <p className="text-xs text-slate-400">
                      Correct Answer: <strong className="text-emerald-400">{q.options[q.correctAnswerIndex]}</strong>
                    </p>
                    <div className="p-3 bg-slate-950/60 rounded-lg text-[11px] text-slate-300 border border-slate-800/80 leading-relaxed">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-800">
                <Button variant="secondary" size="md" icon={RefreshCw} onClick={() => fetchAssessment(assessmentId)}>
                  Retake Checkpoint
                </Button>
                <a href="/app/roadmap">
                  <Button variant="primary" size="md" icon={ChevronRight}>
                    Return to Roadmap
                  </Button>
                </a>
              </div>
            ...
            <div className="space-y-6">
              {assessment.questions?.map((q, qIdx) => (
                <div key={q.id} className="p-5 bg-slate-900/70 border border-slate-800/90 rounded-2xl space-y-3">
                    ...
                    {q.options?.map((opt, oIdx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full p-3.5 rounded-xl border text-left text-xs transition flex items-center gap-3.5 ${
                            isSelected
